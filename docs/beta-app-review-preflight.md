...
- [ ] **Health.** The Watch uses HealthKit only to run a golf workout session for the length of a round. It does not read Health data and does not save the round to Health. Health data does not leave the device. App Privacy: do not declare Health or Fitness as collected. Review notes should say why the golf workout session is there. This needs a new build before it is in a binary.
