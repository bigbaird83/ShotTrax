...
User-facing name is **ShotTraxx™** (`app.json` `expo.name`, iOS `CFBundleDisplayName`, Android `label` / home screen). Bundle ID `com.shottrax.app` and Expo slug `shottrax` stay unchanged (App Store ID). Home-screen icon is the owner flag mark at `assets/images/icon.png` (see `assets/images/README.md`). Splash / launch still is the first frame of Doc’s 3s open clip, not the old fairway wordmark.
...
Cold start still shows the native Expo splash (`splash-icon.png` = first frame of the 3s clip, `resizeMode: contain`, `#000101`). The launch storyboard aspect-fits that 784×1168 frame on the full screen, the same rect as the contained video. After JS boots, `BrandedSplash` plays the open clip (`assets/splash/splash-open-first-3s-v2.mp4`) centered on an opaque `#000101` field. The video is the only artwork during playback — there is no second full-screen still behind it. It hides the native splash on the first frame and plays the clip once. The audio track is removed. Playback is muted at volume 0 with `mixWithOthers`. Tap skips. A 5s safety timeout removes the overlay if playback stalls. It runs on cold start only, not when the app returns from the background. The portrait clip is always `contain` + black letterbox (`#000101`), centered — never `cover` or stretch. Reduce Motion skips the clip and shows the same first-frame still (`assets/splash/splash-first-frame-v2.png`), then the same onDone path.
...
- Watch Connectivity types this cut:
  - Phone → Watch `clubList`: `{ type, top3, bag, labels, holeNumber, yardsToGreen, yardsQuality }` pushed on hole change / fix quality change / bag rank change, and while walking in when the top-3 set or order changes or yards move ≥5 yd (ranking stays on phone). Optional `roundComplete: true` after the last hole is finished. `yardsToGreen` is `yardsToGreen().yards` (`null` when quality is none). `yardsQuality` is `good | soft | none` — same bands as the phone, never invent.
  ...
  - Phone → Watch `puttSheet`: `{ type, open, holeNumber, lengths, labels, canAdd, canMake, done? }` when Putter is selected. `done: true` (Made it / Hole Out) always closes the Watch sheet.
  - Watch → Phone `puttPick`: `{ type, action: add|undo|made, at, lengthId? }` — buckets then **Made it** finishes the hole. The phone then pushes `puttSheet` `done` plus a Hole N+1 `clubList` (or `roundComplete` on the last hole) right away, so the wrist shows the next hole's Suggested clubs, or **Round complete**.
- Watch status: **Hole N · XXX yd** only for a live GPS fix and a loaded green — the same number and gate as phone TO GREEN and the complication (`complicationYards`). **Hole N · —** otherwise, including a weak or missing fix and Catalog only / HARD-MISS (no green). Club-suggest fallback yards stay on `yardsToGreen` for re-rank and do not appear on this line. Tiny **Approximate** chip when that live fix is soft (never SOFT on the wrist).
- Suggested clubs re-rank as you walk in (phone strip + Watch): D is live `yardsToGreen()` when quality is good or soft; quality none keeps the last good/soft D on that hole; else the landing / tee → pin target. Walk-away still ranks from the lie.
...
- Complication: **H{n} · XXX yd** (**H{n} · —** when quality is none or there is no green), via the `ShotTraxxHole` widget. It reads `complicationHole` / `complicationYards` / `complicationQuality` from the app group — live `yardsToGreen(fix, greenCentroid)` only, never the card / tee yardage. The Watch app writes those keys on every clubList and reloads the widget timeline; the phone writes the same keys and, when the complication is on the active face, sends `transferCurrentComplicationUserInfo` (hole / quality flips, ≥20 yd moves) so the face updates while the Watch app is in the background.
...
12. Finish the round for a scorecard. **Nerd out** (Menu, scorecard, summary, or Home) shows score vs par, putts, and club-book carries from saved rounds (live averages from GPS/Placed shots; estimated seeds stay badged Estimated). It also shows **Fairways** (hit rate plus L / R / short misses) and **Greens in reg** for this round and all saved rounds. No strokes gained, no putter. Club averages also live on the Averages tab. Stock wedges are **PW · 48° · 50° · 52° · 56° · 60°** (`club_gw` stays the Gap Wedge id so old rounds keep working; voice still maps gap/gw → `club_gw`).
...
13a. **Fairway / GIR.** On par 4s and 5s a **Fairway?** row (← Left · Hit · Right → · Short) sits on the play map once the tee shot is in, and stays until you answer it. The same row is also in the hole sheet so you can fix it later. Tapping the same choice again clears it. It is one player tap, never guessed from GPS or OSM, and par 3s never ask. **GIR** is computed: on a hole closed with Made it / Hole Out and a real par, strokes before putting (score − putts) ≤ par − 2. A holed approach in regulation counts; a chip-in from off the green does not. Unclosed holes or a missing par leave GIR blank. Fairway taps ride along in round export / restore (`fairway` on each hole; older files import as blank).
13b. **Handicap index** (Nerd out → Handicap). World Handicap System math on saved rounds: score differential = 113 / slope × (adjusted gross score − course rating). Each hole caps at net double bogey (par + 2 + strokes received by stroke index), using the index from the rounds before it; with no index yet the cap is par + 5. The index averages the lowest N of the last 20 differentials (WHS table, with the −2 / −1 / −1 adjustments at 3 / 4 / 6 scores), rounded to the tenth, max 54.0. Two 9-hole rounds pair into one 18-hole score (a 9-hole round on an 18-hole tee uses half the rating). A round counts only when it is finished, every hole has a score and par, and the tee has a rating and slope — missing data never counts. No playing-conditions adjustment, soft/hard cap, or exceptional-score reduction, so it is not an official index. Round stats show the score differential a round contributed.
13c. **Trends** (Nerd out → Trends). Finished rounds only, from saved rows. **Last 5 / 10 / 20** filter. One small bar chart per stat, oldest to newest: vs par, putts, penalty strokes (all per 18 holes, so 9-hole rounds sit on the same scale), fairways hit %, greens in reg %, and carry by club (pick a club). Each card shows the window average and the change vs the same number of rounds before it, with better / worse spelled out, not just colored. A round with nothing stored for a stat is a gap, never a zero. Tap a bar for that round; **Show numbers** lists every value.
13d. **Dispersion** (Nerd out or Club data → Dispersion). Per club, every saved shot measured against the line from where it was hit to that hole's green pin: yards toward the green (up) and yards off the line (right +, left −). Same samples as club averages (GPS good/soft/forced or Placed, no putter, no impossible jumps) plus a saved start, finish, and green. No green, or a start within 20 yd of it, and the shot is skipped — a target line is never guessed. The card shows the average distance and miss, the middle 80% of distances and of left/right misses, and left / on line / right shares (on line = within 5% of the shot's length, at least 3 yd). Tap a dot for its round and hole; **Show numbers** lists every shot and marks placed ones. Placed shots count and may be less accurate than GPS. Doglegs off the tee can read as misses toward the corner.
13e. **Hazard carries.** Under the live GPS → pin badge on the play map, up to three rows like **Bunker R · 212 / 231** and **Water · 180 / 205** (reach / carry yards). They come only from OSM `golf=bunker`, `golf=water_hazard`, and `golf=lateral_water_hazard` outlines for this hole, measured from the live good/soft fix along the line to the green. A hazard counts when part of its outline is within 35 yd of that line, at least 5 yd ahead, and no more than 20 yd past the green. Outline edges are sampled every 3 yd so a wide pond whose corners sit outside the corridor still counts. L / R when the in-play part averages more than 8 yd off line. No fix, no green, poor GPS, a past round, or an unmapped hole → no rows (never a guessed hazard).
14. In-round **Scorecard** is a view only (Menu or the Scorecard button). Holes, par, score, putts. Missing par stays blank. A finished hole (`puttsDone`) never shows a blank score — posted score if set, else logged strokes. A hole you **moved past** without **Made it** / **Hole Out** (`!puttsDone`, close-state only — never GPS) gets a **red outline** and **`!`** in the score column; tap resumes that hole’s play view. Marks: eagle ●, birdie ○, par unmarked, bogey □, double+ □□ — only when both score and par exist. **Share** on this sheet (and Menu) sends the readable scorecard text plus a generated scorecard image. Back returns to the same hole and never marks or closes a shot. **FW** and **GIR** columns: fairway ✓ / ← / → / ↓ (short) from the player tap, GIR ● hit / ○ missed. Blank when unknown. No strokes gained. A small always-on corner badge shows **thru N, ±par** from finished holes only (Made it / Hole Out / puttsDone with a posted or logged score). It hides during Add shot / catch-up and the putt sheet so map gestures stay clear.
