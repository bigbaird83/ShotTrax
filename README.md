...
11. **Putter** opens a putt sheet (phone + Watch) — not a GPS mark. Pick a length bucket (**Under 3 ft · 3–10 · 10–20 · 20+**) — pick does **not** commit. **Add putt** logs that putt; repeat for miss → putt 2. **Made it** closes the hole after the logged putts, **persists hole score = total strokes** (real marks + putts + penalties — never putts-only), and advances (next hole + Pick a club, or summary after the last). **Hole Out** is not on this sheet — it stays on the play dock for off-green chip-ins and writes the same total-stroke score. Putts are stats only — no green GPS and not a map mark. Walking off the green / to the next tee does **not** invent putts. Play continues; a **Finish putts · Hole N** chip stays until you enter them. In-round **Menu** reaches Home, previous hole, and Settings.
