...
async function main() {
  ...
  if (!KEY) {
    ...
    console.log('GCA_GREENS_PRO=NO_KEY');
  ...
  const withCenters = cleanHoles.filter((h) => h.green_center_lat != null && h.green_center_lng != null);
  const greensVerdict =
    greens.status === 403
      ? 'NO greens (403)'
      : greens.status === 200 && withCenters.length > 0
        ? 'YES greens'
        : greens.status === 200
          ? 'NO greens'
          : `NO greens (HTTP ${greens.status})`;
  console.log(`GREENS_VERDICT=${greensVerdict}`);
...
main().catch((e) => {
  const tls = isTlsFailure(e);
  console.log(`GCA_GREENS_PRO=${tls ? 'TLS_FAIL' : 'OTHER'}`);
  console.log('ERROR=' + redact(e?.message || String(e)));
  try {
    mkdirSync(OUT, { recursive: true });
    writeFileSync(
      `${OUT}/report.json`,
      JSON.stringify({ error: tls ? 'TLS_FAIL' : 'OTHER', message: redact(e?.message || String(e)) }, null, 2),
    );
  } catch {
    // ignore write failures in the catch path
  }
  process.exit(0);
});
