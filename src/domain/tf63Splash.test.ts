...
test('TF 63: BrandedSplash plays Doc 3s muted open clip; Reduce Motion uses first-frame still', () => {
  ...
  assert.match(branded, /contentFit="contain"/);
  ...
  const clip = new URL('../../assets/splash/splash-open-3s.mp4', import.meta.url);
  const still = new URL('../../assets/splash/splash-open-still.png', import.meta.url);
