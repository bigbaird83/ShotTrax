 ...
 * putts, and/or penalties, warn when those don't add up to the posted score.
 *
 * No warning when nothing is logged (score-only play) or when score is unset.
 * Putts are hole-finish stats, not map marks.
 */
export function reconcileHoleScore(args: {
  ...
  const puttCount = Math.max(0, args.puttCount ?? 0);
  const penaltyStrokes = Math.max(0, args.penaltyStrokes);
  const logged = shotCount + puttCount + penaltyStrokes;
...
export function scoreMismatchMessage(row: HoleScoreReconcile): string {
  return `Score ${row.score} doesn’t match ${row.shotCount} shots + ${row.puttCount} putts + ${row.penaltyStrokes} penalties.`;
}
