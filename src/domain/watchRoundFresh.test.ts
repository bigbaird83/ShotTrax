...
test('Watch freshness, recovery, and the phone clear are wired', () => {
  ...
  assert.match(session, /applyClubList\(context, fromPhone: true\)/);
  assert.match(session, /applyClubList\(incoming, fromPhone: true, endedByMessage: true\)/);
  assert.match(session, /applyClubList\(incoming, fromPhone: true\)/);
