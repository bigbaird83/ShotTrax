...
test('round screen and summary render ordered penalty steps and keep insert on shots', () => {
  ...
  assert.match(liveHeader, /testID="penalty-shot-chip"/);
  assert.match(liveHeader, /testID="live-hole-count"/);
  assert.match(liveHeader, /formatHoleCountLine/);
  assert.doesNotMatch(liveHeader, /finishedMini\.vsPar/);
  const addPenalty = hole.slice(hole.indexOf('const onAddPenalty'), hole.indexOf('const openBag'));
  assert.ok(addPenalty.indexOf('insertPenalty') < addPenalty.indexOf('bump()'));
  assert.match(addPenalty, /COPY\.penaltySaveFailed/);
  assert.doesNotMatch(addPenalty, /withTransactionSync/);
