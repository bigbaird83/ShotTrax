...
test('TF 54 A/E: Watch Made it is reserved on the putt sheet — always enabled, never clipped off', () => {
  ...
  assert.ok(addUndo.search(/Text\("Made(?: it)?"\)/) < 0);
  assert.ok(watchSheet.indexOf('Text("Add putt")') < watchSheet.search(/Text\("Made(?: it)?"\)/));
  assert.ok(watchSheet.indexOf('Text("Undo")') < watchSheet.search(/Text\("Made(?: it)?"\)/));
